...
# Signing identity. Defaults to ad-hoc so anyone can build from source; the
# maintainer overrides it in a gitignored Local.mk with a real Developer ID —
# and should: a STABLE code identity is what lets the TLS key's Keychain
# approval stick across rebuilds (ad-hoc changes the cdhash every build → a
# password prompt per handshake) and stops local-network TCC re-prompts.
