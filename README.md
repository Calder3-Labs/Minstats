...
Because the phone can *control* the Mac, auth is front-loaded, not bolted on. The full design history — including what failed and why — is in [`docs/SECURITY-HARDENING.md`](docs/SECURITY-HARDENING.md) and [`docs/TLS-PLAN.md`](docs/TLS-PLAN.md).
...
```sh
make run SIGN=-   # build, bundle, ad-hoc sign, install to /Applications, launch
...
```
(`SIGN` defaults to the maintainer's Developer ID; ad-hoc (`SIGN=-`) is fine for your own machines.)
...
docs/                  security hardening history, TLS design
```
...
Known honest limitations: Intel temperatures are unsupported by decision; the iOS app keeps a broad ATS exemption because ATS rejects self-signed certificates on VPN routes even when the pinning delegate approves them (documented, with the exemption-free path sketched, in `docs/TLS-PLAN.md`).
