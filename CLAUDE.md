...
- `make run` — release build, assemble + Developer ID-sign `dist/MinStats.app`
  (hardened runtime; dev builds use the real identity ON PURPOSE — stable code
  identity is what keeps the TLS key prompt-free and local-network TCC sticky),
...
- `Sources/MinStats/Agent/` — the phone-facing agent, embedded in the menu bar
  ...
  - `StatsServer` — ONE NWListener: **HTTPS on 51848 + Bonjour** (agent 2.0.0,
    ...
    `docs/TLS-PLAN.md`) — and with it false the agent does not serve at all.
